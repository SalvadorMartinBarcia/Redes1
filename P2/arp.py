...
cacheLock = Lock()
#Cache de ARP. Es un diccionario similar al estándar de Python solo que eliminará las entradas a los 10 segundos
...
def printCache():
    '''
        Nombre: printCache
        Descripción: Esta función imprime la cache ARP
        Argumentos: Ninguno
        Retorno: Ninguno
    '''
...
def processARPRequest(data, MAC):
    '''
        ...
        Argumentos:
            -data: bytearray con el contenido de la trama ARP (despues de la cabecera común)
    ...
    myIPaux = struct.pack(">I", myIP)

    if ipdestino != myIPaux:
        return

    frame = createARPReply(iporigen, macorigen)
    sendEthernetFrame(frame, len(frame), b'\x08\x06', macorigen)
...
def processARPReply(data,MAC):
        ...
        Decripción: Esta función procesa una respuesta ARP. Esta función debe realizar, al menos, las siguientes tareas:
            ...
            -Comprobar si la IP destino de la petición ARP es la propia IP:
                ...
                -Si es la propia IP:
                    ...
                    -Añadir a la cache ARP la asociación MAC/IP.
    ...
    if requestedIP == None:
        globalLock.release()
        return
    requestedIPaux = struct.pack(">I", requestedIP)
    if iporigen != requestedIPaux:
        globalLock.release()
        return
...
def initARP(interface):
    ...
    #Registrar la función del callback process_arp_frame por cada trama con el Ethertype 0x0806
    registerCallback(process_arp_frame, b'\x08\x06')

    #Obtenemos la dir ip y mac de la interfaz especificada
    myIP = getIP(interface)
    myMAC = getHwAddr(interface)

    #realizar petición ARP gratuita, que consiste en preguntar por la propia dirección IP
...
def ARPResolution(ip):
    '''
        ...
        Descripción: Esta función intenta realizar una resolución ARP para una IP dada y devuelve la dirección MAC asociada a dicha IP
            o None en caso de que no haya recibido respuesta. Esta función debe realizar, al menos, las siguientes tareas:
                ...
                -Si está en cache devolver la información de la cache
    ...
    if ip in cache:
	    ...
	    resolvedMAC = None
	    globalLock.release()
	    sendEthernetFrame(frame, len(frame), b'\x08\x06', broadcastAddr)
