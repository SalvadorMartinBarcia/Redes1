...
def process_Ethernet_frame(us,header,data):
    ...
    #primeros 6 bytes, dir destino, 6bytese origen, 2 bytes tipo de ethernet
    destino = data[0:6]
    origen = data[6:12]
    ...
    if destino != broadcastAddr and destino != macAddress:
        return
    if ethertype.hex() in upperProtos:
        funcion = upperProtos.get(ethertype.hex())
        funcion(us, header, data[14:], origen)
    else:
        return
...
def registerCallback(callback_func, ethertype):
    ...
    upperProtos[ethertype.hex()] = callback_func
...
def startEthernetLevel(interface):
    ...
    handle = None
    if levelInitialized: #Si ya está inicializado devuelve -1
        return -1
    macAddress = getHwAddr(interface) #Obtenemos la interfaz
    #abrimos la interfaz en modo promiscuo
    errbuf = bytearray()
    handle = pcap_open_live(interface, ETH_FRAME_MAX, PROMISC, TO_MS, errbuf)
    if handle == None:
        sprint('Error al capturar la interfaz')
        return -1
    ...
    #todo es correcto, marcamos como true la variable global de nivel
...
def stopEthernetLevel():
    ...
    recvThread.stop() #paramos el hilo
    pcap_close(handle) #Cerramos la interfaz
    levelInitialized = False
    macAddress = None
    return 0
...
def sendEthernetFrame(data, len, etherType, dstMac):
    ...
    #rellenamos la trama con data, macaddress, dstmac y ethertype
    frame = bytes()
    frame += dstMac
    frame += macAddress
    frame += etherType
    ...
    if total_len > ETH_FRAME_MAX:
        return -1
    if total_len < ETH_FRAME_MIN:
        i = ETH_FRAME_MIN - total_len
        frame += b'\x00'*i
        total_len = 60

    ret = pcap_inject(handle,frame, total_len) #añadir la trama y poner su tamaño
    if ret == None:
        sprint('Error al enviar la trama')
        return -1
